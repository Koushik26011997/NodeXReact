...
import {
  ...
  Image,
  Pressable,
  TouchableOpacity as TouchableBody,
} from 'react-native';
...
const Users = (props: any) => {
  ...
  const renderItem = (item: any, index: number) => {
    return (
      ...
      <TouchableOpacity
        ...
        onPress={() => props.navigation.navigate('AddUser', {id: item._id})}>
        <Rtext fontSize={15} fontWeight="bold" style={{marginBottom: 3}}>
          {'#' + (index + 1) + '. ' + item.name}
        </Rtext>
        <Rtext fontSize={14} style={{marginBottom: 3}}>
          {item.email}
        </Rtext>
        <Rtext fontSize={14} style={{marginBottom: 3}}>
          {item.dob}
  ...
  const getAllUsers = async () => {
    try {
      ...
      setItemList(response.data.data.reverse() || []);
    } catch (e) {
    ...
    <View style={styles.backgroundStyle}>
      ...
      {itemList.length > 0 ? (
        ...
        <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
          <Rtext color="#000" fontSize={14}>
            Sorry, No users(s) found.
          </Rtext>
        </View>
      )}
